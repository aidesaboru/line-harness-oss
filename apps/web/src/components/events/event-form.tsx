...
const EVENT_SLOT_BULK_EMPTY_MESSAGE = '生成される予約枠が0件でした。条件を確認してください。'
const EVENT_COPY_ERROR_MESSAGE = 'コピーに失敗しました。表示されているURLを選択してコピーしてください。'
...
export default function EventForm({ accountId, eventId }: EventFormProps) {
  ...
  async function copyValue(v: string) {
    ...
    setError(EVENT_COPY_ERROR_MESSAGE)
  }
...
function SlotsTab({
  ...
  async function deleteSlot(slotId: string) {
    ...
    const ok = await requestConfirm({
      title: '予約枠を削除しますか？',
      message: '既存予約がある枠は削除できません。対象日時を確認してから実行してください。',
      confirmLabel: '削除',
      tone: 'danger',
    })
    if (!ok) return
        ...
        <BulkSlotDialog
          ...
          onSubmit={async (input) => {
            ...
            if (generated.length === 0) {
              throw new Error(EVENT_SLOT_BULK_EMPTY_MESSAGE)
            }
            const ok = await requestConfirm({
              title: '予約枠を一括生成しますか？',
              message: `${generated.length}件の予約枠を生成します。日付・曜日・時刻パターンに間違いがないか確認してください。`,
              confirmLabel: '生成',
              tone: 'warning',
            })
            if (!ok) return

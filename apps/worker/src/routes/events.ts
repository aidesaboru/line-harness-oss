...
function parseEventIdempotencyKey(raw: string | undefined): string | null {
  if (!raw) return null;
  const key = raw.trim();
  if (!key || key.length > EVENT_IDEMPOTENCY_KEY_MAX_LENGTH) return null;
  // Header values should stay compact and log-safe; UUIDs generated by the
  // LIFF client are accepted by this visible-ASCII token set.
...
events.post('/api/liff/events/:id/bookings', async (c) => {
  ...
  if (!rawIdemKey) return bad(c, 'idempotency_key_required', 400);
  const idemKey = parseEventIdempotencyKey(rawIdemKey);
  if (!idemKey) return bad(c, 'invalid_idempotency_key', 400);
  const callerLineUserId = await verifyCallerLineUserId(c.req.header('Authorization'), c.env);
  if (!callerLineUserId) return bad(c, 'unauthorized', 401);

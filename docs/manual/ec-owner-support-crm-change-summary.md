...
- raw chat/staff/friends/booking/event UI error searchesで、チャット一覧/詳細/過去読み込み/送信/ローディング/ステータス/メモ保存、友だち詳細サイドバー、友だち一覧、予約管理、イベント予約管理、予約スタッフ、シフト、予約メニュー、メニュー割当、イベント一覧、イベントフォーム、予約枠操作、スタッフ管理がAPI応答文字列やraw exceptionを直接表示しないことを確認
- コンソールエラーは0件

...
- LIFFイベント予約/サロン予約導線の `prompt(` / `confirm(` / `alert(`、raw `String(err/e)`、raw `err.message`、raw console warn/error検索は0件。LIFF起動失敗画面のBrowser smokeでは固定案内だけが表示され、`channel not found` やtoken系文字列は表示されない
- コンソールエラーは0件

...
- `apps/worker/src/routes/entry-routes.ts` / `conversions.ts` / `calendar.ts` / `health.ts`: 流入経路、conversion point定義参照/作成/削除、Google Calendar接続、account health/migration管理APIのowner/admin制限

...
- Customer/friend data access changed? `Yes`: staff chat/inbox/users-grouped/users/account-settings/conversions/calendar/conversation visibility and direct friend API access are limited to friends tied to visible support cases; broadcast preview/count/progress/insight/dedup-preview, admin recent messages/friend debug/repair APIs, form submission lists, booking/event admin, rich menu catalog/group management, scenario/reminder/scoring/tag/template/message-template definition management, automation/auto-reply/notification rule/traffic pool/operator management, and Stripe/ad/affiliate/tracked-link management APIs are owner/admin-only; fixture candidate output does not print friend names or case titles by default.

...
- Web support-meta/clipboard/staff-form tests verify manual editor validation, copy fallback behavior, staff creation payload validation, and stable user-facing staff management failure messages; raw chat/staff UI error searches verify chat list/detail/history/send/loading/status/notes and staff management failures do not directly display API error strings or raw exceptions; support/staff pages route destructive actions through the shared in-app confirmation dialog before API calls.

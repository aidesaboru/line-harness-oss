...
corepack pnpm --filter worker test -- src/routes/management-role-guards.test.ts # 33 tests
...
- Management role guard tests verify account health/migration failure logs and error responses keep only exception kind or fixed internal error, without account IDs, migration IDs, token-like text, or raw exception messages.

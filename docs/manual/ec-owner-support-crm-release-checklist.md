...
- [ ] template/message-template定義payloadがDB書き込み前に検証される
  - Worker content management route testsでは、壊れたJSON、不正なname/category/messageType/messageContent、壊れたFlex/image JSON、空updateがDB writeや既存template lookup前に400で止まり、正常payloadはtrimされることを確認済み。

...
- イベント予約の拒否理由入力と友だち追加シナリオ作成名入力は、ブラウザ標準 `prompt` ではなく共通の画面内入力ダイアログ (`useTextInputDialog`) を使う。残る `prompt` は、クリップボードAPIが使えない場合のコピーfallbackだけに限定する。

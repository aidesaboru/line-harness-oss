...
- [ ] tag/template/message-template定義path/payloadがDB書き込み前に検証される
  - Worker content management route testsでは、不正なtag削除path ID、壊れたJSON、不正なname/color/category/messageType/messageContent、壊れたFlex/image JSON、空updateがDB writeや既存template lookup前に400で止まり、正常値はtrimされることを確認済み。
